...
class RaceScene extends Phaser.Scene {
    ...
    create() {
        ...
        const availColors = allAiColors.filter(c => c !== GameState.getPlayerColor());
        const aiConfigs = [
        ...
        this.money = 0;
        this.lastWaypointIndex = this.track.startLineIndex || 0;       // track progress around the loop
        this.expectedWaypoint = (this.track.startLineIndex + 1) % this.track.waypoints.length;        // next waypoint we expect (forward only)
    ...
    spawnPickups() {
        ...
        const vehicleScale = this.playerTruck ? Math.max(0.8, this.playerTruck.truckWidth / 10) : 1;
        const pickupScale = vehicleScale;
        const coinSpread = Math.min(trackWidth * 0.28, Math.max(18, 16 * pickupScale));
        const nitroSpread = Math.min(trackWidth * 0.22, Math.max(14, 12 * pickupScale));
        ...
        for (let i = 0; i < 12; i++) {
            ...
            const nx = -dy / segLen;
            const ny = dx / segLen;
            const offset = (Math.random() - 0.5) * coinSpread;
            const coin = new Coin(this, x + nx * offset, y + ny * offset, 'money', pickupScale);
        ...
        for (let i = 0; i < 3; i++) {
            ...
            const nx = -dy / segLen;
            const ny = dx / segLen;
            const offset = (Math.random() - 0.5) * nitroSpread;
            const nitro = new Coin(this, x + nx * offset, y + ny * offset, 'nitro', pickupScale);
    ...
    updateWaypointTracking() {
        ...
        // Detect wrong way using actual movement direction, not just truck heading
        ...
        const moveSpeed = Math.sqrt(truck.vx * truck.vx + truck.vy * truck.vy);
        const moveAngle = moveSpeed > 0.01 ? Math.atan2(truck.vy, truck.vx) : truck.angle;
        let angleDiff = moveAngle - toExpAngle;
        // Normalize to -PI..PI
        while (angleDiff > Math.PI) angleDiff -= Math.PI * 2;
        while (angleDiff < -Math.PI) angleDiff += Math.PI * 2;
        // Wrong way if moving mostly away from the expected direction
        this.goingWrongWay = moveSpeed > 30 && Math.abs(angleDiff) > (Math.PI * 2 / 3);
    ...
    _assignPodiumSlot(truck) {
        ...
        const pos = this._getPodiumSlotPosition(slot);
        truck._podiumTarget = pos;
        this._drawPodiumSlot(slot, pos.x, pos.y);
    }

    _getTrackDistanceToCenter(px, py) {
    ...
    _getPodiumSlotPosition(slot) {
        const waypoints = this.track.waypoints;
        const startWp = waypoints[this.track.startLineIndex];
        const nextWp = waypoints[(this.track.startLineIndex + 1) % waypoints.length];
        const dx = nextWp[0] - startWp[0];
        const dy = nextWp[1] - startWp[1];
        const len = Math.sqrt(dx * dx + dy * dy) || 1;
        const nx = -dy / len;
        const ny = dx / len;
        const offsetDist = Math.max(this.track.trackWidth * 0.5 + 80, 100);
        const posA = { x: startWp[0] + nx * offsetDist, y: startWp[1] + ny * offsetDist };
        const posB = { x: startWp[0] - nx * offsetDist, y: startWp[1] - ny * offsetDist };
        const base = this._getTrackDistanceToCenter(posA.x, posA.y) > this._getTrackDistanceToCenter(posB.x, posB.y) ? posA : posB;
        const forwardX = dx / len;
        const forwardY = dy / len;
        const slotSpacing = Math.round(45 * Math.max(1, (this.track.trackWidth || 70) / 70));
        return {
            x: base.x + forwardX * slot * slotSpacing,
            y: base.y + forwardY * slot * slotSpacing
        };
    }

    _drawPodiumSlot(slot, px, py) {
        ...
        const truckScale = Math.max(0.8, this.track.truckScale || 1);
        const scale = Math.min(1.4, Math.max(0.9, truckScale));
        const bw = Math.round(50 * scale);
        const bh = Math.round(heights[slot] * scale);
        const labelSize = Math.max(8, Math.round(10 * scale));

        // Podium block
        gfx.fillStyle(colors[slot], 0.9);
        gfx.fillRect(px - bw / 2, py + 10, bw, bh);
        gfx.lineStyle(Math.max(1, Math.round(2 * scale)), 0xffffff, 0.5);
        gfx.strokeRect(px - bw / 2, py + 10, bw, bh);

        // Position label
        const label = this.add.text(px, py + 10 + bh / 2, labels[slot], {
            fontSize: `${labelSize}px`,
            fontFamily: "'Press Start 2P', cursive",
            color: '#000000'
        }).setOrigin(0.5).setDepth(901);

        this._podiumVisuals.push({ gfx, label });
    }

    _clearPodiumVisuals() {
        for (const visual of this._podiumVisuals) {
            if (visual.gfx) visual.gfx.destroy();
            if (visual.label) visual.label.destroy();
        }
        this._podiumVisuals = [];
    }
    ...
    finishRace() {
        ...
        // Reassign podium slots based on final sorted results
        this._clearPodiumVisuals();
        this._podiumNextSlot = 0;
        for (let i = 0; i < Math.min(results.length, 3); i++) {
            const result = results[i];
            const truck = result.isPlayer ? this.playerTruck : this.aiTrucks[result.index];
            const pos = this._getPodiumSlotPosition(i);
            truck._podiumTarget = pos;
            this._drawPodiumSlot(i, pos.x, pos.y);
            this._podiumNextSlot++;
        }
        ...
        for (let i = 0; i < trucks.length; i++) {
            for (let j = i + 1; j < trucks.length; j++) {
                ...
                const minDist = Math.max(
                    (Math.max(a.truckWidth, a.truckHeight) + Math.max(b.truckWidth, b.truckHeight)) * 0.55,
                    18
                );
        ...
        for (const truck of this.allTrucks) {
            ...
            for (const h of this.track.hazards) {
                ...
                if (h.type === 'water') {
                    ...
                    // Push AWAY from center — moderate effect, but less aggressive so trucks can recover.
                    if (!truck._bumpCooldown || truck._bumpCooldown <= 0) {
                        const t = dist / h.radius;
                        const bumpStr = Math.sin(Math.PI * t) * (1 - t * 0.2);
                        truck.speed *= (1 - 0.18 * bumpStr);
                        const away = Math.atan2(dy, dx);
                        let push = away - truck.angle;
                        while (push > Math.PI) push -= Math.PI * 2;
                        while (push < -Math.PI) push += Math.PI * 2;
                        const maxPush = Math.PI * 0.5;
                        push = Math.max(-maxPush, Math.min(maxPush, push));
                        truck.angle += push * bumpStr * 0.4;
                        const nudge = h.radius * 0.08 * bumpStr;
                        truck.x += Math.cos(away) * nudge;
                        truck.y += Math.sin(away) * nudge;
                        if (truck === this.playerTruck) {
                            this.cameras.main.shake(100, 0.004 * bumpStr);
                        }
                        truck._bumpCooldown = 0.5;
                    }
                } else if (h.type === 'oil') {
