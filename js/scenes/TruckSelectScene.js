...
class TruckSelectScene extends Phaser.Scene {
    ...
    create() {
        ...
        const colors = Object.keys(presets)
            .map(Number)
            .filter(color => GameState.isVehicleUnlocked(color));
        const cols = 3;
    ...
    _drawTruckCard(color, preset, x, y, w, h) {
        ...
        const tw = isBike ? 18 : 24;
        const th = isBike ? 30 : 40;
        const tcx = cx, tcy = y + 65;

        if (isBike) {
            truckGfx.fillStyle(0x111111, 1);
            truckGfx.fillRect(tcx - 2, tcy - 16, 4, 8);
            truckGfx.fillRect(tcx - 2, tcy + 8, 4, 8);
            truckGfx.fillStyle(0x777777, 1);
            truckGfx.fillRect(tcx - 1, tcy - 14, 2, 4);
            truckGfx.fillRect(tcx - 1, tcy + 10, 2, 4);

            truckGfx.fillStyle(0x000000, 1);
            truckGfx.fillRoundedRect(tcx - tw / 2, tcy - th / 2, tw, th, 3);
            truckGfx.fillRoundedRect(tcx - 6, tcy - 8, 12, 16, 5);
            truckGfx.fillStyle(0x222222, 1);
            truckGfx.fillRoundedRect(tcx - 5, tcy - 12, 10, 6, 3);
            truckGfx.fillStyle(0x555555, 1);
            truckGfx.fillRoundedRect(tcx - 3, tcy - 2, 6, 8, 3);
            truckGfx.fillStyle(0x222222, 0.7);
            truckGfx.fillRoundedRect(tcx - 3, tcy - 12, 6, 4, 2);
            truckGfx.fillStyle(0x333333, 1);
            truckGfx.fillRect(tcx - 7, tcy - 12, 3, 1);
            truckGfx.fillRect(tcx + 4, tcy - 12, 3, 1);
            truckGfx.fillRect(tcx - 6, tcy - 12, 1, 4);
            truckGfx.fillRect(tcx + 5, tcy - 12, 1, 4);
        } else {
            // Body
